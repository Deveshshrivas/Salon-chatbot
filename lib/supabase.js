...
export async function getActiveConversations() {
  ...
  q = q.order('last_inbound_at', { ascending: false, nullsFirst: false }).limit(150)
...
// Handover (freeze/resume) is routed through WF-3 → WF-2, NOT a direct RPC: the
// `freeze_conversation` / `resume_conversation` functions are not exposed to the
// anon key (calling them from the browser returns PGRST202 "function not found"),
// so the webhook runs them server-side with service-role access and also fires
// WF-2's customer-facing handover steps. Accepts a conversation object (preferred,
// so we can pass wa_id/phone_number_id/channel) or a bare id.
export async function freezeConversation(conversation, employeeId = null, reason = 'staff_takeover') {
  ...
  if (!res || res.success === false) return { error: res?.error || 'freeze_failed' }
  return res
}
...
export async function resumeConversation(conversation, outcome = 'resolved', notes = null) {
  ...
  if (!res || res.success === false) return { error: res?.error || 'resume_failed' }
  return res
}
